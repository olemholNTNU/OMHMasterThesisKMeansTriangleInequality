...
// Elkans Modified
int findClosestCenter(Point centers[], long* centerdistances, long* lowerbounds, long* upperbound, Point newPoint) {
	// since we now remember what this point previously was, we dont calculate the distance anew.
	long shortestDistanceCenter[2] = { *upperbound, newPoint.center };
	for(int i=0; i<currentAmountOfCenters; i++) {
		//Compare-Means Modification, but also Lemma 1 of Elkans method
		if (i == shortestDistanceCenter[1] || centerdistances[shortestDistanceCenter[1]*currentAmountOfCenters+i] >= 2*shortestDistanceCenter[0]) {
			continue;
		}
	...
	*upperbound = shortestDistanceCenter[0];
...
int main(int argc, char *argv[]) {
	...
	long * centerDistances = (long*) malloc(sizeof(long));
	long * shortestCD = (long*) malloc(sizeof(long));

	long * lowerbounds = (long*) malloc(sizeof(long));
	long * upperbounds = (long*) malloc(sizeof(long));
...
//	Due to currently reading file needs to be updated to take the next set of input.
	for (int i=0; j<batchSize && !feof(inputData) && fscanf(inputData, "%ld", &newPoint.coord[i]);) {
		...
		if (c == '\n') {
			...
			if (currentAmountOfCenters >= maxCenterAmount) {
				...
				lowerbounds = (long*) realloc(lowerbounds, sizeof(long)*(currentAmountOfPoints+1)*currentAmountOfCenters);
				for (int e=0; e<currentAmountOfCenters; e++) {
					lowerbounds[currentAmountOfPoints*currentAmountOfCenters+e] = 0;
				}

				upperbounds = (long*) realloc(upperbounds, sizeof(long)*(currentAmountOfPoints+1));
				...
				centerDistances = realloc(centerDistances, sizeof(long)*currentAmountOfCenters*currentAmountOfCenters);
				shortestCD = realloc(shortestCD, sizeof(long)*currentAmountOfCenters);
				...
				lowerbounds = (long*) realloc(lowerbounds, sizeof(long)*currentAmountOfPoints*currentAmountOfCenters);
				for (int e=0; e<currentAmountOfCenters; e++) {
					lowerbounds[(currentAmountOfPoints-1)*currentAmountOfCenters+e] = 0;
				}

				upperbounds = (long*) realloc(upperbounds, sizeof(long)*currentAmountOfPoints);
	...
	// An alterantive is to add a for loop that increases itterations and records results
	...
	for (int i=0; i<currentAmountOfCenters; i++) {
		previousCenters[i].coord = (long*) calloc(sizeof(long), amountOfCoord);
	}
